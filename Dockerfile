...
# 将工作目录指定为与项目代码位置一致
WORKDIR /go/src/code.meikeland.com/me/forwarding
# 将代码从代码库复制到打包环境的WORKDIR
COPY . .
# 将main文件，从cmd中复制到WORKDIR的根目录
COPY cmd/forwarding/main.go ./main.go

# 将访问阿里云code的ssh复制到打包环境
...
# 复制部署内容
COPY --from=golang-builder /go/src/code.meikeland.com/me/forwarding/app .
COPY --from=golang-builder /go/src/code.meikeland.com/me/forwarding/docs ./docs
# 仅当程序中包含静态模板文件时进行复制
# COPY --from=golang-builder /go/src/code.meikeland.com/me/forwarding/template ./template
